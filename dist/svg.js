import { CELL_SIZE, DELTA_TIME, GAP_SIZE, GHOSTS, GRID_HEIGHT, GRID_WIDTH, PACMAN_COLOR, PACMAN_COLOR_DEAD, PACMAN_COLOR_POWERUP, WALLS } from './constants.js';
import { Utils } from './utils.js';
const generateAnimatedSVG = (store) => {
    // Reduzir logs para apenas informações essenciais
    if (store.frameCount < 10) {
        console.log("Estado inicial dos fantasmas:", store.ghosts.map(g => ({
            name: g.name,
...
const generateGhostsPredefinition = () => {
    ...
    ['blinky', 'inky', 'pinky', 'clyde'].forEach(ghostName => {
        ...
        ['up', 'down', 'left', 'right'].forEach(direction => {
            const ghostObj = GHOSTS[ghostName];
    ...
    <symbol id="ghost-scared" viewBox="0 0 ${CELL_SIZE} ${CELL_SIZE}">
        <image href="${GHOSTS['scared'].imgDate}" width="${CELL_SIZE}" height="${CELL_SIZE}"/>
    </symbol>`;
    // Adicionar os olhos do fantasma (para cada direção)
    ['up', 'down', 'left', 'right'].forEach(direction => {
        if (GHOSTS['eyes'] && direction in GHOSTS['eyes']) {
            const eyesObj = GHOSTS['eyes'];
            defs += `
